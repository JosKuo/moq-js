	...
	constructor(init: Uint8Array) {
		...
		this.#mp4.onReady = (info: MP4.Info) => {
			...
			for (const track of info.tracks) {
				this.#mp4.setExtractionOptions(track.id, track, { nbSamples: 1 }) // extract data from track X, nb samples is the number os samples to extract
			}
		}

		this.#mp4.onSamples = (_track_id: number, track: MP4.Track, samples: MP4.Sample[]) => {
			//console.log("boxes", this.#mp4.boxes);

			for (const sample of samples) {
				const prfts = this.#getLastPRFTs() as MP4.BoxParser.prftBox[];
				// Store sample and associated PRFT data
				this.#samples.push({ track, sample, prfts });
